...
```{r}
 ggplot(data = homerange, mapping = aes(x = log10.preymass, y = log10.hra)) + geom_point() +geom_smooth(method=lm, se=T)


```
It removed the missing values.
...
mamha %>%
  ...
  ggplot(aes(x = common.name, y=mean.mass.g)) +geom_bar(stat="identity")+coord_flip()
```
