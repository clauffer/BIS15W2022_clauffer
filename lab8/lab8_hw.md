...
```
## here() starts at /Users/colinlauffer/Desktop/BIS15W2022_clauffer
```
...
We are assigning multiple pieces of the same data to the same variable so it is not tidy. The data is currently in wide format
...
```r
sydney_long <- sydenybeaches_wide %>%
  pivot_longer(-site, 
              names_to = "data",
              values_to = "values")
sydney_long
...
```
South Maroubra Rockpool was the most polluted
